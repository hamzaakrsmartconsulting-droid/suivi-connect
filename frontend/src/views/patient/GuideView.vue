...
import {
  Rocket, LayoutDashboard, ClipboardList, Pill, CalendarDays,
  MessageSquare, Bell, FileBarChart2, HeartPulse,
} from '@lucide/vue'
...
const sections: { id: string; label: string; icon: Component }[] = [
  ...
  { id: 'suivi',       label: 'Suivi hebdomadaire',    icon: ClipboardList },
  { id: 'medicaments', label: 'Médicaments',           icon: Pill },
  { id: 'rdv',         label: 'Rendez-vous',           icon: CalendarDays },
  { id: 'messages',    label: 'Messages',              icon: MessageSquare },
  { id: 'alertes',     label: 'Alertes',               icon: Bell },
  { id: 'rapports',    label: 'Rapports & ordonnances', icon: FileBarChart2 },
]
</script>
...
<template>
  <div class="guide-page">
    <div class="guide-hero">
      <Pictogram :icon="HeartPulse" tone="brand" size="lg" />
      <div>
        <h1 class="guide-hero__title">Guide patient</h1>
        <p class="guide-hero__sub">Tout ce que vous devez savoir pour utiliser SuiviConnect</p>
      </div>
    </div>
    ...
    <div class="guide-layout">
      ...
      <div class="guide-content">
        <section v-if="active === 'debut'">
          ...
          <p class="sec-desc">Connectez-vous pour accéder à votre espace personnel de suivi cardiaque.</p>
          <div class="tip-card">
            <Pictogram :icon="ClipboardList" tone="teal" size="sm" />
            <p>Chaque semaine, renseignez votre suivi : cela aide votre médecin à adapter votre traitement.</p>
          </div>
        </section>

        <section v-else-if="active === 'tableau'">
          <h2 class="sec-title">Tableau de bord</h2>
          <p class="sec-desc">Consultez poids, tension, LDL, activité et votre score de risque en un coup d’œil.</p>
        </section>

        <section v-else-if="active === 'suivi'">
          <h2 class="sec-title">Suivi hebdomadaire</h2>
          <p class="sec-desc">Questionnaire médical court : mesures, symptômes et adhésion au traitement.</p>
        </section>

        <section v-else-if="active === 'medicaments'">
          <h2 class="sec-title">Médicaments</h2>
          <p class="sec-desc">Retrouvez vos prescriptions et rappels de prises.</p>
        </section>

        <section v-else-if="active === 'rdv'">
          <h2 class="sec-title">Rendez-vous</h2>
          <p class="sec-desc">Confirmez ou annulez vos consultations planifiées par votre médecin.</p>
        </section>

        <section v-else-if="active === 'messages'">
          <h2 class="sec-title">Messages</h2>
          <p class="sec-desc">Échangez de façon sécurisée avec votre médecin, y compris en appel si besoin.</p>
        </section>

        <section v-else-if="active === 'alertes'">
          <h2 class="sec-title">Alertes</h2>
          <p class="sec-desc">Soyez informé lorsque certains indicateurs nécessitent une attention.</p>
        </section>

        <section v-else>
          <h2 class="sec-title">Rapports &amp; ordonnances</h2>
          <p class="sec-desc">Téléchargez vos rapports PDF et consultez vos ordonnances.</p>
        </section>
      </div>
    </div>
  </div>
